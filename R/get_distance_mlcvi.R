...
# pairwise lookup
.mlcvi_pair <- function(from, to, data = NULL, label = "MLCVI") {
  ...
  if (!is.matrix(data)) {
    return(list(value = NA_real_, error = paste0("ERROR (", label, "): data is not a matrix/dist/data.frame")))
  }

  # Must have dimnames
  rn <- rownames(data); cn <- colnames(data)
  if (is.null(rn) || is.null(cn)) {
    return(list(value = NA_real_, error = paste0("ERROR (", label, "): matrix must have row and column names")))
  }

  from <- .clean_iso3_maybe(from); to <- .clean_iso3_maybe(to)

  if (!(from %in% rn)) {
    return(list(value = NA_real_, error = paste0("ERROR (", label, "): country code not found (row): ", from)))
  }
  if (!(to %in% cn)) {
    return(list(value = NA_real_, error = paste0("ERROR (", label, "): country code not found (col): ", to)))
  }

  # Direct lookup; if your matrix is symmetric, this already covers it
  val <- suppressWarnings(as.numeric(data[from, to]))
  if (is.na(val)) {
    return(list(value = NA_real_, error = paste0("ERROR (", label, "): missing value for ", from, " -> ", to)))
  }
...
# print result for a user-supplied matrix
.matrix_report <- function(from, to, res, digits = 3) {
  from <- .clean_iso3_maybe(from); to <- .clean_iso3_maybe(to)
  fmt <- if (is.null(res$error)) sprintf(paste0("%.", digits, "f"), res$value)
  else paste0("NA\n  ", res$error)
